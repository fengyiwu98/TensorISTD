function TLC = myTLC2(file_path, ext_name, img_idx)
...
for k = -2: 2
    ...
    if k < 0
        ...
        motionVect(:,k+3) = get_motionVect_rand2(img_cur, temp, 64, 9, -8*k);
    end
    if k > 0
        frame_data(:,:,k+2) = double(temp);
        motionVect(:,k+2) = get_motionVect_rand2(img_cur, temp, 64, 9, 8*k);
    end
