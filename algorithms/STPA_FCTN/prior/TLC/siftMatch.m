...
% colormap('gray');
% imagesc(img3);title('Result in Siftmatch');
% hold on;
% cols1 = size(img1,2);
% for i = 1: size(des1,1)
%   if (matchTable(i) > 0)
...
matchLoc1 = [x1,y1];
matchLoc2 = [x2,y2];
% figure;subplot(1,2,1);imshow(img1);hold on;plot(x1,y1,'r.'); %此处得到的点是经过筛选提出后用于变换参数估计的点
% subplot(1,2,2);imshow(img2);hold on;plot(x2,y2,'g.');
