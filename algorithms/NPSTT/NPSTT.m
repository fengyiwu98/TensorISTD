...
%end
    ...
    N = ones(1,1+round(dim(1,3)/2));  % [I_3 / 2] + 1 涓厓绱?
    
    for iter = 1 : max_iter

        preT = sum(S(:) > 0);  % 鐩爣涓潪0鍏冪礌鐨勪釜鏁?
        
        % update L 鑳屾櫙B
        R =  -S+X-Y/mu;
        L = obj.t_CSVT(R,N,mu);  % TCNN绠楀瓙浣滅敤浜嶳浠ユ洿鏂癓锛圢涓哄垎娈靛寮傚?涓暟锛宼au = 1/mu锛?
        % 璁＄畻鍒嗘濂囧紓鍊间釜鏁帮紙涓?杩唬涓瘡涓垏鐗囩殑鍒嗘浣嶇疆閮戒笉鍚岋級
        for index = 1:1+round(dim(1,3)/2)
        %for index = 1:8
             [~,S,~] = svd(L(:,:,index),'econ');
             diagS = diag(S);
             thresh = max(diagS-threshold, 0);
             % 鍒嗘濂囧紓鍊间釜鏁癗(1,index)璁句负L涓ぇ浜巘hreshold鐨勫寮傚?涓暟
...
% 寮犻噺涓婇檺濂囧紓鍊奸槇鍊肩畻瀛?t-CSVT)
% N(1,i)涓哄垎娈靛寮傚?涓暟锛宼au = 1/mu
...
% first frontal slice 绗竴涓闈㈠垏鐗?
% 闄嶅簭SVD
[U,S,V] = svd(Y(:,:,1),'econ');
diagS = diag(S);
[desS, sIdx] = sort(diagS, 'descend');
%.[Y1, Y2, Y3, ...] = deal(X1, X2, X3, ...)鐩稿綋浜?Y1 = X1; Y2 = X2; Y3 = X3; ...
[desU, desV] = deal(U(:, sIdx), V(:, sIdx));
% 浠涓虹晫闄愶紝灏嗛檷搴忕殑USV鍒囨垚鍓嶄竴鍗婂拰鍚庝竴鍗?
...
% 涓嬪紡绛変环浜嶽U1,U2]*diag([diagS1,threshS2])*[V1,V2]'
