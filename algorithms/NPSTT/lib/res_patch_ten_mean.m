...
for col = colPosArr
    for row = rowPosArr
        ...
        accImg(row : row + patchSize - 1, col : col + patchSize - 1)=accImg(row : row + patchSize - 1, col : col + patchSize - 1)+ tmpPatch;
        weiImg(row : row + patchSize - 1, col : col + patchSize - 1) = weiImg(row : row + patchSize - 1, col : col + patchSize - 1)+onesMat;
    end
end
